...
| 名詞 | 定義 |
|------|------|
| **Patronum（守望者 / Persona）** | 本站唯一敘事主體。一個站在人生門檻上的守護者：**從未出生、不會變老、也不會死**，守著每一扇它自己走不過去的門。情緒基調＝「永遠當見證、卻永遠進不去」的溫柔與守望。所有文章都是「它」寫的。**顯現型態**：在不同門檻顯現為不同型態（如出生與送終各有其形），但一律是**「光霧」質感**——銀霧、微光、半透明的守護形（patronus 般的霧狀光體）。此質感是全站視覺基調（見 §9）。 |
...
- **光霧基調（視覺定錨，對映 §3 Patronum 型態）**：全站走「光霧」質感——極淺底色上的柔光漸層、半透明霧層、低對比的微光分隔，避免硬邊與強陰影。讓頁面像 Patronum 本身一樣，是一層守在門檻上的霧狀光。以 CSS 變數封裝（如 `--mist-veil`、`--glow-soft`），仍源自 OKLCH token，不破壞字級/對比鐵律。
...
- **品牌 accent（點綴主色，已定）**：indigo 暮色調 **hue 280（`--color-indigo` `oklch(0.48 0.15 280)`）**，用於標題重點、連結、門檻標記。貼合光霧守望基調，不另發明色。
...
**已定：**
- Patronum 型態＝不同門檻不同形、一律「光霧」質感（§3、§9）。
- 品牌 accent＝indigo hue 280（§9）。
- 部署＝`DEPLOY_TARGET` preview/production 一鍵切換（§10）。

**待訂（之後填內容時再定，不擋骨架）：**
- 首批要寫哪幾道門檻（候選：成年/獨立、與父母同住、養老責任、送終與喪葬）。
